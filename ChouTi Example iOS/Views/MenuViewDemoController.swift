...
extension MenuViewDemoController: MenuViewDataSource {
    func numberOfMenusInMenuView(_: MenuView) -> Int {
        return 10
    }

    func menuView(_: MenuView, menuViewForIndex index: Int, contentView: UIView?) -> UIView {
        let label = UILabel()
...
extension MenuViewDemoController: MenuViewDelegate {
    func menuView(_: MenuView, menuWidthForIndex _: Int) -> CGFloat {
        return 50
    }

    func menuView(_: MenuView, didSelectIndex selectedIndex: Int) {
        print("did selected: \(selectedIndex)")
    }

    func menuView(_: MenuView, didScrollToOffset offset: CGFloat) {
